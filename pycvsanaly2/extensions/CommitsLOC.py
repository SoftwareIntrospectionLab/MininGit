...
class DBCommitLines(object):
    ...
    __insert__ = """INSERT INTO commits_lines (id, commit_id, added, removed) 
                    values (?, ?, ?, ?)"""
...
class SVNLineCounter(LineCounter):
    ...
    diffstat_pattern = re.compile("^ \d+ file[s]? changed(, (\d+) " + \
        "insertion[s]?\(\+\))?(, (\d+) deletion[s]?\(\-\))?$")
    ...
    def get_lines_for_revision(self, revision):
        ...
        pipe = Popen(self.diffstat, shell=False, stdin=PIPE, stdout=PIPE, 
...
class GitLineCounter(LineCounter):
    ...
    diffstat_pattern = re.compile("^ \d+ files changed(, (\d+) " + \
    ...
    def __init__(self, repo, uri):
        ...
        try:
            ...
            raise ExtensionRunError("Error running CommitsLOC extension: %s", 
                                    str(e))
...
_counters = {'cvs': CVSLineCounter, 'svn': SVNLineCounter, 
             'git': GitLineCounter}
...
def create_line_counter_for_repository(repo, uri):
    try:
        ...
        raise ExtensionRunError("Repository type %s is not supported by " + \
                                "CommitsLOC extension" % (repo.get_type()))
...
class CommitsLOC(Extension):
    ...
    def run(self, repo, uri, db):
        ...
        try:
            ...
            cursor.execute(statement("SELECT max(id) from commits_lines", 
                                     db.place_holder))
        ...
        while rs:
            ...
            if commit_list:
                ...
                write_cursor.executemany(statement(DBCommitLines.__insert__, 
                                        self.db.place_holder), commits_lines)
